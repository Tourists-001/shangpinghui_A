            ...
            <tr>
              <th colspan="5">
                <span class="ordertitle"
                  ...
                  <span class="pull-right delete" @click="deleteOrder(order)"
...
export default {
  ...
  methods: {
    async getData() {
      ...
      const res = await this.$API.reqMyOrderList(pageNo, pageSize);
    ...
    deleteOrder(items) {
      // console.log(items);
      this.myOrder.records.forEach((item, i) => {
        if (item === items) {
          this.myOrder.records.splice(i, 1);
        }
      });
    },
  },
